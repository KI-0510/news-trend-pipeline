...
def gemini_insight(api_key: str, model: str, context: Dict[str, Any],
    ...
    if not api_key:
        ...
        if total_days >= 2:
            try:
                ...
                diff = 0
        return (
            f"(로컬 요약) 최근 {total_days}일 흐름을 기준으로 간단 요약합니다. "
            f"마지막 일자 증감은 {diff}건입니다. 상위 토픽은 산업·제품·정책 축으로 분포하며, "
            f"실행 액션은 1) 상위 토픽 사례 수집, 2) 급증 키워드 원인 파악, 3) 파트너십/조달 여부 검토가 적절합니다."
        )
    ...
    try:
        ...
        return f"(요약 생성 실패: {e}) 최근 흐름과 상위 토픽을 바탕으로 우선순위 과제를 정리하세요."
