...
def plot_keyword_network(keywords, docs, out_path="outputs/fig/keyword_network.png",
    ...
    if not docs or not vocab:
        plt.figure(figsize=(8, 5)); plt.text(0.5, 0.5, "키워드 네트워크 생성 불가(데이터 부족)", ha="center", va="center")
        plt.axis("off"); plt.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(); return {"nodes": 0, "edges": 0}
    def _tok(x: str):
    ...
    co = Counter(); freq = Counter()
    for toks in token_docs:
        toks_in = [t for t in toks if t in vocab]
        for w in toks_in: freq[w] += 1
        for i in range(len(toks_in)):
            for j in range(i + 1, len(toks_in)):
                a, b = sorted((toks_in[i], toks_in[j])); co[(a, b)] += 1
    edges = [(a, b, c) for (a, b), c in co.items() if c >= min_cooccur]
    edges = sorted(edges, key=lambda x: x[2], reverse=True)[:max_edges]
    G = nx.Graph()
    for w, f in freq.items():
        if f > 0 and (w in vocab) and (w and len(w) >= 2): G.add_node(w, freq=f)
    for a, b, c in edges:
        if a in G.nodes and b in G.nodes: G.add_edge(a, b, weight=c)
    if G.number_of_nodes() == 0 or G.number_of_edges() == 0:
        plt.figure(figsize=(8, 5)); plt.text(0.5, 0.5, "키워드 네트워크 생성 불가(엣지 없음)", ha="center", va="center")
        plt.axis("off"); plt.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(); return {"nodes": 0, "edges": 0}
    try:
        comms = list(nx.algorithms.community.greedy_modularity_communities(G))
        comm_map = {}; 
        for ci, com in enumerate(comms):
            for n in com: comm_map[n] = ci
        num_comm = max(comm_map.values()) + 1 if comm_map else 1
    except Exception:
        comm_map = {n: 0 for n in G.nodes()}; num_comm = 1
    ...
    if len(w_raw) > 5:
        q95 = np.quantile(w_raw, 0.95); w_raw = np.minimum(w_raw, q95)
    W_MIN, W_MAX = 0.8, 2.2
    ...
    for n in label_nodes:
        ...
        if not txt: continue
        x, y = pos[n]
        ax.text(x, y, txt, ha="center", va="center", fontsize=8, color="#111111", zorder=5, clip_on=False,
                fontname=font_name, bbox=dict(boxstyle="round,pad=0.20", fc="white", ec="none", alpha=0.80))
